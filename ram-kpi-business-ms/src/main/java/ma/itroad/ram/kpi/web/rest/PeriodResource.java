...
/**
 * REST controller for managing {@link ma.itroad.ram.kpi.domain.Period}.
 */
@RestController
...
public class PeriodResource {
    ...
    public ResponseEntity<PeriodDTO> createPeriod(@Valid @RequestBody PeriodDTO periodDTO) throws URISyntaxException {
        ...
        if (periods.size() > 0) {
            PeriodDTO existingPeriodDTO = periods.get(0);
            return ResponseEntity.status(HttpStatus.CONFLICT).header(HttpHeaders.LOCATION, "/business/api/periods/" + existingPeriodDTO.getId()).build();
        }
    ...
    public ResponseEntity<PeriodDTO> updatePeriod(
        ...
        return ResponseEntity
                .ok()
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, periodDTO.getId().toString()))
                .body(result);
    }
    ...
    public ResponseEntity<PeriodDTO> partialUpdatePeriod(
        ...
        return ResponseUtil.wrapOrNotFound(
                result,
                HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, periodDTO.getId().toString())
        );
    }
