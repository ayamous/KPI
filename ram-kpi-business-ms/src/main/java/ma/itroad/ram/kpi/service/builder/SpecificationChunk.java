package ma.itroad.ram.kpi.service.builder;
...
public class SpecificationChunk<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> cquery, CriteriaBuilder cbuilder) {
        ...
        switch (condition.getOperation()) {
            ...
            case FOREIGN_KEY:
                return cbuilder.equal(
                        root.get(condition.getLeftHand()).get("id"), condition.getRightHand());
           /* case EQUAL_ENUM:
                return cbuilder.equal(
                        root.get(condition.getLeftHand()), condition.getEnumRightHand());*/
            default:
                return null;
        }
    }
